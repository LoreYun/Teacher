...
{    
    ...
    if (_scrollView.isDragging) {
        ...
        }else if (_state == MJRefreshStateNoMore)
        {
...
{
    if (_state != MJRefreshStateRefreshing && _state != MJRefreshStateNoMore) {
        // 存储当前的contentInset
        _scrollViewInitInset = _scrollView.contentInset;
    }
    ...
    // 3.存储状态
    if (_state != MJRefreshStateNoMore) {
        _state = state;
    }
}
