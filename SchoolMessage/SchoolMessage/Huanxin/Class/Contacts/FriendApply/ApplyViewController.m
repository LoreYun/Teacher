...
- (void)viewDidLoad
{
    ...
    if (!self.friendInfoData) {
        self.friendInfoData = [NSMutableDictionary dictionary];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(self.dataSource.count > indexPath.row)
    {
        ...
        __weak ApplyViewController *controller = self;
        if (entity) {
            ...
            else if (applyStyle == ApplyStyleJoinGroup)
            {
                ...
                FriendInfo *info = [self.friendInfoData objectForKey:entity.applicantUsername];
                NSLog(@"ApplyCOn friend:%@",info);
                NSString *content = entity.reason;
                if(info){
                    NSString *nick = info.nickName.length>0?info.nickName:entity.applicantNick?entity.applicantNick:entity.applicantUsername;
                    cell.titleLabel.text = nick;
                    [cell.headerImageView setImageWithURL:[NSURL URLWithString:info.headPhotoUrl] placeholderImage:[UIImage imageNamed:@"chatListCellHead"]];
                    
                    if([content rangeOfString:entity.applicantUsername].location!=NSNotFound)
                    {
                        content = [content stringByReplacingOccurrencesOfString:entity.applicantUsername withString:@""];
                    }
                    
                }else{
                    cell.titleLabel.text = entity.applicantNick?entity.applicantNick:entity.applicantUsername;
                    [[RosterManager sharedInstance] getFriendInfoByChatter:entity.applicantUsername callBack:^(BOOL succeed, FriendInfo *firendInfo) {
                        if (succeed) {
                            if (!entity.applicantUsername) {
                                return ;
                            }
                            [controller.friendInfoData setObject:firendInfo forKey:firendInfo.chatter];
                            [controller.tableView reloadData];
                        }
                    }];
                    [cell.headerImageView setImageWithURL:[NSURL URLWithString:@""] placeholderImage:[UIImage imageNamed:@"chatListCellHead"]];
                }
