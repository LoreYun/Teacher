...
- (EMSearchDisplayController *)searchController
{
    if (_searchController == nil) {
        ...
        [_searchController setCellForRowAtIndexPathCompletion:^UITableViewCell *(UITableView *tableView, NSIndexPath *indexPath) {
            ...
            NSString *imageName = info.group.isPublic ? @"groupPublicHeader" : @"groupPrivateHeader";
            [cell.imageView setImageWithURL:[NSURL URLWithString:info.groupHeadPhotoUrl] placeholderImage:[UIImage imageNamed:imageName]];  //.image = [UIImage imageNamed:imageName];
            if (info.group.groupSubject && info.group.groupSubject.length > 0) {
                cell.textLabel.text = info.group.groupSubject;
            }
            else {
                cell.textLabel.text = info.group.groupId;
            }
        ...
        [_searchController setDidSelectRowAtIndexPathCompletion:^(UITableView *tableView, NSIndexPath *indexPath) {
            ...
            GroupInfo *info = [weakSelf.searchController.resultsSource objectAtIndex:indexPath.row];
            ChatViewController *chatVC = [[ChatViewController alloc] initWithChatter:info.group.groupId isGroup:YES];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    GroupInfo *info = [self.dataSource objectAtIndex:indexPath.row];
    ChatViewController *chatController = [[ChatViewController alloc] initWithChatter:info.group.groupId isGroup:YES];
    chatController.title = info.group.groupSubject;
...
{
    __weak GroupListViewController *con = self;
